<template>
    <div>
        <CCard>
            <CCardHeader>
            <slot name="header">
                <CIcon name="cil-list"/>Clock In/Out
            </slot>
            </CCardHeader>
            <CCardBody>
                <CRow>
                    ...
                    <CDataTable
                        ...
                        <template #timeEntry="data">
                            <td>
                                ...
                                <template v-if="data.item.timeEntry.clockOut">
                                  -
  ...
  methods: {
    getTimeEntrys() {
      ...
      if(this.selectedPatrol.enableScheduling){
        this.timeEntryFields = [{key:'date',label:'Date'},
          {key:'timeEntry',label:'Entry'},
          {key:'shiftTimes',label:'Shift Time'},
          {key:'user',label:'User'},
      ...
      this.$store.dispatch('loading','Loading...');
        this.$http.post('timeclock/entries',{patrolId:this.selectedPatrolId,userId:this.selectedUserId,from:this.from,to:this.to})
            .then(response => {
